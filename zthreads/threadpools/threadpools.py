...
def action(thread_name,arg,logger=None):
    """
    真实的任务定义在这个函数里
    ...
    import random
    result = {'values': random.random()}
    return result
 
 
class Threadpools:
 
    def __init__(self, max_num, max_task_num=None,logger=None):
        """
        初始化线程池
    ...
    def call(self):
        ...
        while event != StopEvent:
            ...
            try:
                ...
                success = False
                if self.logger!=None:
                    self.logger.error('Call function-{0} exception: {1}'.format(func,e))
            # 如果有指定的回调函数
            if callback is not None:
                ...
                try:
                    ...
                    #pass
                    if self.logger!=None:
                        self.logger.error('callback function exception: {0}'.format(e))
